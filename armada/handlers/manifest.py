        ...
        * A manifest with schema "armada/Manifest/v*"
        ...
        if len(manifests) > 1:
            error = ('Multiple manifests are not supported. Ensure that the '
                     '``manifest`` option is set to specify the desired '
                     'manifest.')
            LOG.error(error)
        ...
        if not all(self.charts, self.groups, self.manifest):
            expected_schemas = [
                DOCUMENT_CHART, DOCUMENT_GROUP, DOCUMENT_MANIFEST
            ]
            ...
            raise ValueError(error % expected_schemas)

    def _find_documents(self, desired_manifest=None):
        charts = groups = manifests = []
        for document in self.documents:
            ...
            if document.get('schema') == DOCUMENT_MANIFEST:
                ...
                if desired_manifest and manifest_name == desired_manifest:
