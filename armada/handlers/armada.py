...
class Armada(object):
    ...
    def sync(self):
        ...
        for chart_group in self.config[KEYWORD_ARMADA][KEYWORD_GROUPS]:
            ...
            stub = None
            if group_wait:
                stub = self.tiller.gen_install_stub()
            ...
            for raw_chart in chart_group.get(KEYWORD_CHARTS, []):
                ...
                # process install
                    ...
                    #    process.start()
                    #else:
                    install_info = self.tiller.install_release(protoc_chart,
                                                                prefix_chart,
                                                                chart.namespace,
                                                                dry_run=self.dry_run,
                                                                values=yaml.safe_dump(values),
                                                                wait=individual_wait,
                                                                timeout=chart_timeout)
                    status = self.tiller.get_release_status(install_info.release.name)
                    LOG.info(status.info.status.resources)
