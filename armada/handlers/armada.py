...
class Armada(object):
    ...
    def tag_cloned_repo(self, ch, repos):
        ...
        if ct_type == 'local':
            ...
            reference = ch.get('chart').get('source').get('reference', 'master')
            ...
            if repo_branch not in repos.keys():
                try:
                    ...
                    raise source_exceptions.GitLocationException('{} branch: {}'.format(*repo_branch))
                repos[repo_branch] = repo_dir
                ch.get('chart')['source_dir'] = (repo_dir, subpath)
            else:
                ch.get('chart')['source_dir'] = (repos.get(repo_branch), subpath)
        else:
