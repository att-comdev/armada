...
class ChartBuilder(object):
    ...
    def get_files(self):
        ...
        def _append_file_to_result(root, relFolderPath, file):
            abspath = os.path.abspath(os.path.join(root, file))
            relpath = os.path.join(relFolderPath, file)
        ...
        for root, dirs, files in os.walk(self.source_directory):
            ...
            relFolderPath = os.path.relpath(root, self.source_directory)

            if relfolder not in ['charts', 'templates']:
                for file in files:
                    if (file not in files_to_ignore and
                            file not in non_template_files):
                        _append_file_to_result(root, relFolderPath, file)
            elif relfolder == 'charts' and '.prov' in files:
                _append_file_to_result(root, relFolderPath, '.prov')
