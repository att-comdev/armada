...
class Tiller(object):
    ...
    def delete_resources(self, release_name, resource_name, resource_type,
        ...
        # TODO would like to prepend something like the following
        #       if/when we add release label to all resources
        # label_selector = 'release_group={}, '.format(release_name)

        label_selector = ', '.join([list(label.keys())[0] + '=' +
                                    list(label.values())[0]
        ...
        if 'job' in resource_type:
            ...
            for job in jobs.items:
                ...
                self.delete_resources(release_name, resource_name, 'pod',
                                      resource_labels, namespace)
