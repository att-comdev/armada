...
class Tests(api.BaseResource):
    ...
    def _check_validation_response(self, resp, result, details):
        resp.content_type = 'application/json'
        resp_body = {
            'kind': 'Status',
            'apiVersion': 'v1.0',
    ...
    def _validate_documents(self, resp, documents):
        result, details = validate.validate_armada_documents(documents)
        return self._check_validation_response(resp, result, details)

    def _validate_manifests(self, resp, documents):
        result, details = validate.validate_armada_manifests(documents)
        return self._check_validation_response(resp, result, details)
    ...
    def on_post(self, req, resp):
        ...
        is_valid = self._validate_documents(resp, documents)
        if not is_valid:
            err_message = 'Failed to validate documents.'
            self.error(req.context, err_message)
            return self.return_error(
                resp, falcon.HTTP_400, message=err_message)

        is_valid = self._validate_manifests(resp, documents)
        if not is_valid:
            err_message = ('Failed to validate manifest generated from '
                           'documents.')
            self.error(req.context, err_message)
            return self.return_error(
                resp, falcon.HTTP_400, message=err_message)
