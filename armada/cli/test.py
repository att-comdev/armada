...
To obtain tiller service status/information:
    ...
    $ armada tiller --release-name blog-1
...
@click.option('--release-name', help='armada manifest', type=str)
@click.option('--tiller-host', help="Tiller host ip")
@click.option(
    '--tiller-port', help="Tiller host port", type=int, default=44134)
@click.pass_context
def test_charts(ctx, file, release_name, tiller_host, tiller_port):
    TestChartManifest(file, release_name, tiller_host, tiller_port).invoke()


class TestChartManifest(CliAction):
    def __init__(self, file, release_name, tiller_host, tiller_port):
    ...
    def invoke(self):
        ...
        if self.release_name:
            self.logger.info("RUNNING: %s tests", self.release_name)
            resp = tiller.testing_release(self.release_name)

            if not resp:
                self.logger.info("FAILED: %s", self.release_name)
                return

            test_status = getattr(resp.info.status, 'last_test_suite_run',
                                  'FAILED')
            if test_status.results[0].status:
                self.logger.info("PASSED: %s", self.release_name)
            else:
                self.logger.info("FAILED: %s", self.release_name)
