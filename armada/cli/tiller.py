...
def tiller_service(ctx, tiller_host, tiller_port, releases, status):
    TillerServices(tiller_host, tiller_port, releases, status).invoke()


class TillerServices(CliAction):

    def __init__(self, tiller_host, tiller_port, releases, status):
    ...
    def invoke(self):
        ...
        if self.status:
            ...
            self.logger.info(
                'Tiller Version: %s', getattr(resp.Version, 'sem_ver', False))
