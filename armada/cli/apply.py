...
@click.option('--wait',
              ...
              is_flag=True)
@click.option('--desired-manifest',
              help=('The desired manifest to run. Useful for specifying which '
...
def apply_create(ctx, locations, api, disable_update_post, disable_update_pre,
                 ...
                 timeout, values, wait, desired_manifest, debug):
    ...
    ApplyManifest(ctx, locations, api, disable_update_post, disable_update_pre,
                  dry_run, enable_chart_cleanup, set, tiller_host, tiller_port,
                  timeout, values, wait, desired_manifest).invoke()
