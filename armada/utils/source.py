...
def git_clone(repo_url, ref='master', auth_method=None):
    ...
    try:
        if auth_method and auth_method.lower() == 'ssh':
            ...
            ssh_clone_timeout = 60
            start_time = time.time()
            original_exc = None
            attempt = 1

            while time.time() - start_time < ssh_clone_timeout:
                try:
                    LOG.debug('Attempt #%d at cloning the repository via SSH.',
                              attempt)
                    repo = git.Repo.clone_from(
                        repo_url, temp_dir, env=env_vars)
