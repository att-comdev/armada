...
def test_chartbuild_get_templates(mocker):
    ...
    # Scenario 1: can extract templates from chart using helmignore
    mock_source_path = mocker.patch.object(ChartBuilder, 'get_source_path')
    mock_source_path.return_value = base_manifest
    chartbuilder = ChartBuilder(chart)

    assert len(chartbuilder.get_templates()) == 4

    # Scenario 2: invalid path we return empty array
    chartbuilder = ChartBuilder(chart)
    chartbuilder.source_directory = ''

    assert len(chartbuilder.get_templates()) == 0

    # Scenario 3: skip templates if file is ignored.
    mock_ignore_file = mocker.patch.object(ChartBuilder, 'ignore_file')
    mock_ignore_file.return_value = True
...
def test_chartbuild_get_helm_chart(mocker):

    document = yaml.load(chart_stream)
    document_2 = yaml.load(chart_stream)
    document_2['dependencies'] = []
    document['dependencies'] = [{'chart': document_2}]
    chart = dotify(document)
    ...
    chartbuilder._helm_chart = helm_chart

    assert chartbuilder.get_helm_chart() == helm_chart
